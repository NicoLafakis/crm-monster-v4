// Module loader: import the CRMMonster module and initialize it on DOM ready.
import CRMMonster from './src/crm-monster.js';

function initializeCRMMonster() {
    // Attach to window for runtime console helpers and backwards compatibility
    if (!window.crmMonster) {
        window.crmMonster = new CRMMonster();
    }
    window.CRMMonster = CRMMonster;
}

if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeCRMMonster);
} else {
    initializeCRMMonster();
}
// HubSpot CRM Monster Application
class CRMMonster {
    constructor() {
        ...
        // Service layer: small, optional integration point. The new `src/api-service.js`
        // attaches a global `window.apiService`. If present, expose it on the instance.
    ...
    executeTool(toolName) {
        ...
        const toolConfigs = {
            ...
            'api-tester': {
                ...
                onExecute: () => this.simulateAPITest()
            }
        };
        ...
        return `
            <div class="tool-form">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" rows="6" placeholder='{"properties": {}}'></textarea>
                </div>
                <button class="btn--primary btn--full-width" onclick="window.crmMonster.executeCurrentTool()">
    ...
    simulateMerge() {
        ...
        const interval = setInterval(() => {
            ...
            if (progress >= 100) {
                clearInterval(interval);
                setTimeout(() => {
                    // Neutral behavior: reduce duplicates by a small amount if present
                    this.hubspotData.duplicates = Math.max(0, this.hubspotData.duplicates - 1);
                    this.updateDashboardStats();
                    this.showSuccessModal('Merge Complete!', 'Merged duplicate contacts. Please review remaining duplicates.');
                    this.addActivity('Duplicate merge completed', 'Duplicates merged', 'success');
                    this.isProcessing = false;
                }, 500);
            }
        }, 400);
        ...
        const interval = setInterval(() => {
            ...
            if (progress >= 100) {
                ...
                setTimeout(() => {
                    ...
                    this.hubspotData.duplicates = newDuplicates;
                    this.updateDashboardStats();
                    this.showSuccessModal('Scan Complete!', `Duplicate scan finished. Review results for potential duplicates.`);
                    this.addActivity('Duplicate scan completed', 'Duplicate scan finished', 'info');
                    this.isProcessing = false;
                }, 500);
            }
        }, 250);
    ...
    simulateAPITest() {
        ...
        setTimeout(() => {
            // Neutral result: indicate the test completed. Real responses are shown when ApiService is configured.
            this.showSuccessModal('API Test Finished', 'API test completed. Configure the API service for live responses.');
            this.isProcessing = false;
        }, 800);
    }
